...
_common_attrs = ["spacecraft", "startdate", "enddate", "starttime",
...
_psp_attrs = ["p_dyn", 'psp_epilo_e', 'psp_epilo_p', 'psp_epihi_e',
             ...
             'psp_epilo_ic_viewing', 
              "psp_ch_het_e", "psp_ch_het_p", "psp_ch_epilo_pe", "psp_ch_epilo_ic"]
...
class Options:
    def __init__(self):
        ...
        self.spacecraft = w.Dropdown(value=None, description="Spacecraft", options=["Parker Solar Probe", "Solar Orbiter", "L1 (Wind/SOHO)", "STEREO"], style=_style)
        ...
        self.starttime = w.TimePicker(description="Start time:", value=dt.time(0,0), step=60, style=_style)
        self.endtime = w.TimePicker(description="End time:", value=dt.time(0,0), step=60, style=_style)

        self.resample = w.BoundedIntText(value=10, min=0, step=1, description='Averaging (min):', disabled=False, style=_style)
        self.resample_mag = w.BoundedIntText(value=10, min=0, step=1, description='MAG averaging (min):', disabled=False, style=_style)
        self.resample_stixgoes = w.BoundedIntText(value=10, min=0, step=1, description="STIX/GOES averaging (min):", style=_style)
        ...
        self.psp_epilo_p = w.Checkbox(description="EPI-Lo protons/ions", value=True)
        self.psp_epihi_e = w.Checkbox(description="EPI-Hi electrons", value=True)
        self.psp_epihi_p = w.Checkbox(description="EPI-Hi protons/ions", value=True)
        self.psp_epihi_p_combine_channels = w.Checkbox(description="Combine EPI-hi ion channels", value=True)
        self.psp_het_viewing = w.Dropdown(description="HET viewing", options=["A", "B"], style=_style)
        self.psp_epilo_viewing = w.Dropdown(description="EPI-Lo PE (e) viewing:", options=range(0,8), style=_style, disabled=False, value=3)       
        self.psp_epilo_ic_viewing = w.Dropdown(description="EPI-Lo IC (p) viewing:", options=range(0,80), style=_style, disabled=False, value=3)
        self.psp_epilo_channel = w.Dropdown(description="EPI-Lo PE channel", options=['F', 'E', 'G'], style=_style, disabled=True, value='F')
        self.psp_epilo_ic_channel = w.Dropdown(description="EPI-Lo IC channel", options=['T', 'D', 'R', 'P', 'C'], style=_style, disabled=True, value='T')
        self.psp_ch_het_e = w.SelectMultiple(description="EPI-hi HET electrons", options=range(0,18+1), value=tuple(range(0,18+1,3)), rows=10, style=_style)
        self.psp_ch_het_p = w.SelectMultiple(description="EPI-hi HET protons/ions", options=range(0,14+1), value=tuple(range(0,14+1,2)), rows=10, style=_style)
        ...
        self.solo_ch_het_e = w.SelectMultiple(description="HET electrons:", options=range(0,3+1), value=tuple(range(0,3+1,1)), style=_style)
        self.solo_ch_ept_p = w.SelectMultiple(description="EPT protons/ions", options=range(0,30+1), value=tuple(range(0,30+1,5)), rows=10, style=_style)
        self.solo_ch_het_p = w.SelectMultiple(description="HET protons/ions", options=range(0,35+1), value=tuple(range(0,35+1,5)), rows=10, style=_style)
        ...
        self.l1_ch_erne_p = w.SelectMultiple(description="ERNE p energy channels:", options=range(0, 9+1, 1), value=tuple(range(0,9+1,2)), rows=10, disabled=False, style=_style)
        ...
        self.ster_sept_viewing = w.Dropdown(description="SEPT viewing", options=['sun', 'asun', 'north', 'south'], style=_style)
        
        self.ster_ch_sept_e = w.SelectMultiple(description="SEPT electrons", options=range(0,14+1), value=tuple(range(0,14+1, 2)), rows=10, style=_style)
        self.ster_ch_sept_p = w.SelectMultiple(description="SEPT protons", options=range(0,29+1), value=tuple(range(0,29+1,4)), rows=10, style=_style)
        self.ster_ch_het_p =  w.SelectMultiple(description="HET protons:", options=range(0,10+1), value=tuple(range(0,10+1,2)), rows=10, style=_style)
        self.ster_ch_het_e = w.SelectMultiple(description="HET electrons:", options=(0, 1, 2), value=(0, 1, 2), style=_style)
        ...
        self._commons = w.HBox([w.VBox([getattr(self, attr) for attr in _common_attrs]), w.VBox([getattr(self, attr) for attr in _variable_attrs])])
...
def energy_channel_selection():
    ...
    if options.spacecraft.value == "Parker Solar Probe":
        return psp.energy_channel_selection()
    
    if options.spacecraft.value == "Solar Orbiter":
        return solo.energy_channel_selection()

    if options.spacecraft.value == "L1 (Wind/SOHO)":
        return l1.energy_channel_selection()
    
    if options.spacecraft.value == "STEREO":
        display(stereo.energy_channel_selection())

        if options.ster_sept_e.value == True:
            selection.append(options.ster_ch_sept_e)
            
        if options.ster_sept_p.value == True:
            selection.append(options.ster_ch_sept_p)

        if options.ster_het_e.value == True:
            selection.append(options.ster_ch_het_e)

        if options.ster_het_p.value == True:
            selection.append(options.ster_ch_het_p)
